...
class ThingSpeakClass
{
  public:
	...
	int writeFields(unsigned long channelNumber, const char * writeAPIKey)
	{
		...
		Serial.print("Made postMessage: "); Serial.println(postMessage);
		delay(500);
		Serial.println("Calling writeRaw(num, msg, key)...");
		delay(500);
		return writeRaw(channelNumber, postMessage, writeAPIKey);
	};
	...
	{
		Serial.println("In writeRaw()"); delay(500);
		Serial.print("ts::writeRaw   (channelNumber: "); Serial.print(channelNumber); delay(500);
		Serial.print(" writeAPIKey: "); Serial.print(writeAPIKey); delay(500);
		Serial.print(" postMessage: \""); Serial.print(postMessage); Serial.println("\")");delay(500);
		...
		Serial.println("Did connect to server! :)"); delay(500);
		...
		Serial.println("Wrote headers to client");
		if(!this->client->print(postMessage)) return abortWriteRaw();
		Serial.println("Wrote postmessage to client");
...
private:
	...
	{
		Serial.println("In connectTS()"); delay(500);
		bool connectSuccess = false;
		if(this->customIP == INADDR_NONE && NULL == this->customHostName)
        {
  			    Serial.print("               Connect to default ThingSpeak IP...");delay(500);
			    connectSuccess = client->connect(THINGSPEAK_IPADDRESS,THINGSPEAK_PORT_NUMBER);
			Serial.print("After connect call"); Serial.println(connectSuccess); delay (500);
            if(!connectSuccess)
            {
  			        Serial.print("Failed. Try default URL...");delay(500);
			connectSuccess = client->connect(THINGSPEAK_URL,THINGSPEAK_PORT_NUMBER);
			Serial.print("After IP connect call"); Serial.println(connectSuccess); delay (500);
            }
       }
       else
       {
